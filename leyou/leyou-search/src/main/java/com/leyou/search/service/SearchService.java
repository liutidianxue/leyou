...
public class SearchService {
    ...
    //json工具，可以序列化为json字符串,json字符串反序列化你需要的类型
    ...
    public Goods buildGoods(Spu spu) throws IOException {
        ...
        skus.forEach(sku -> {
            ...
            //获取sku中的图片，数据库的图片，可能是多张，多张是以“，”分割，所以也以逗号来切割返回图片数组，获取第一张图片
        ...
        params.forEach(param -> {
            ...
            if(param.getGeneric()){
                ...
                //如果是特殊的规格参数，从specialSpecMao中获取值
    ...
    private List<Brand> getBrandAggResult(Aggregation aggregation) {
        ...
        return terms.getBuckets().stream().map(bucket -> {
           //bucket.getKeyAsNumber().longValue()  知道是long类型的，所以不用getkey得到object类型，转为为number类型再转化为long类型
